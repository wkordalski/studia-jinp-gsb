...
class AccountID {
	...
	public:
		...
		template<typename T>
		AccountID(const T &t) : _valid(false) {}
...
class Transfer : public Operation {
	protected:
		...
		std::string _msg;
	public:
		Transfer(Date date, Money money, acc_id_type from,
			acc_id_type to, std::string msg) :
...
public:
	AccountSettings(double monthlyCharge,
...
class Account {
	...
	protected:
		...
		Account(acc_id_type account_id, id_type bank_id, 
				...
				Currency currency = Currency::ENC) :
			_id(account_id),
			_balance(0.0),
			bank_id(bank_id),
			settings(settings),
...
class Bank : public InterstellarClockObserver {
	...
	public:
		...
		Bank(std::string name, id_type id,
			...
			_name(name), _id(id),
			checking_settings(checking), saving_settings(saving),
		...
		CurrencyAccount & openCurrencyAccount(const Being &who, Currency currency) {
			auto *ptr = new CurrencyAccount({_id, accounts.size()},
